...
/**
 * Basic functions to Estupendo/Stun application.
 *
 * This file define basic functions for some use on the environment.
 *
 ...
 *
 */
function stunSiteInfo($title=null) {
    $siteInfo = array(
         'title'        => ($title == null ? STUN_TITLE : $title . ' - ' . STUN_TITLE),
         'url'          => STUN_URL,
         'theme'        => THEME,
         'theme_url'    => STUN_URL . '/content/themes/' . THEME
    );
    return $siteInfo;
}

function stunAdminInfo($title=null) {
    $adminInfo = array(
         'title'        => ($title == null ? 'Admin | ' . STUN_TITLE : $title . ' - Admin | ' . STUN_TITLE),
         'url'          => STUN_URL . '/admin',
         'template_url' => STUN_URL . '/app/admin/template'
    );
    return $adminInfo;
}

/** Limit the number of characters in a string */
function stunLimText($string, $length, $append="...") {
	$string = trim($string);
...
/** Create a random string with any length */
function stunRandomString($length) {
	return substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyz"), 0, $length);
}

/** Uploads files to local path */
function stunUploadCloud($file, $ext, $folder) {
 ...
 * @return string
 */
function stunSlugify($str, $options = array()) {
